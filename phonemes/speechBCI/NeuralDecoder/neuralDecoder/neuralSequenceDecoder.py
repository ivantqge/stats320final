...
class NeuralSequenceDecoder(object):
    ...
    def __init__(self, args):
        ...
        model_type = self.args['model'].get('type', 'GRU')  # Default to GRU for backwards compatibility
        
        if model_type == 'TTT_RNN':
            # Initialize TTT-RNN model
            self.model = models.TTT_RNN(
                ...
                ttt_config=self.args['model'].get('ttt_config', {}),
                use_enhanced_ttt=self.args['model'].get('use_enhanced_ttt', False)
            )
        else:
            # Initialize standard GRU model (default)
