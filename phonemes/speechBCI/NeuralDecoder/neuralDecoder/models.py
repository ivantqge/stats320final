...
from .ttt_models import TTT_RNN
    ...
    def call(self, x, states=None, training=False, returnState=False):
        ...
        if self.bidirectional:
            for i, rnn in enumerate(self.rnnLayers):
                result = rnn(x, training=training, initial_state=states[i])
                # For bidirectional GRU, safely unpack the result
                try:
                    x, forward_s, backward_s = result
                    new_states.append([forward_s, backward_s])
                except ValueError:
                    # If unpacking fails, try alternative approaches
                    x = result[0]
                    try:
                        # Try unpacking just the states
                        states_tuple = result[1:]
                        if states_tuple:
                            new_states.append(list(states_tuple))
                        else:
            ...
            for i, rnn in enumerate(self.rnnLayers):
                result = rnn(x, training=training, initial_state=states[i])
                # For unidirectional GRU, safely unpack the result
                try:
                    x, s = result
                    new_states.append(s)
                except ValueError:
                    # If unpacking fails, extract manually
                    x = result[0]
                    try:
                        s = result[1] if result[1:] else None
                        new_states.append(s)
                    except:
                        new_states.append(None)
