...
class NeuralTrainer:
    ...
    def train(self):
        ...
        self.log_to_wandb({
            "model/total_parameters": sum([np.prod(v.shape) for v in self.model.trainable_variables]) if hasattr(self.model, 'trainable_variables') and self.model.trainable_variables else 0
        })
...
def create_default_config():
    ...
    return {
        ...
        'data': {
            ...
            'train_split': 0.8,
            'max_files': 2  # Start with just 2 files for debugging
        },
        'model': {
